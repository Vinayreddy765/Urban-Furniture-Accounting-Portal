...
router.get('/',ctrl.list); router.post('/',[body('name').trim().notEmpty(),body('type').isIn(['Income','Expense'])],validate,ctrl.create); router.put('/:id',authorize('Administrator'),ctrl.update); router.patch('/:id/archive',authorize('Administrator'),ctrl.archive);
