...
const mapProduct = (p) => ({ ...p, salesPrice: n(p.sales_price ?? p.salesPrice), cost: n(p.cost_price ?? p.cost), archived: !!p.is_archived });
...
const mapJournal = (x) => ({ ...x, defaultAccountId: x.default_account_id ?? x.defaultAccountId, archived: !!x.is_archived });
const mapAnalytic = (x) => ({ ...x, id: x.id, name: x.name, type: x.type });
const mapBudget = (x) => ({ ...x, id: x.id, name: x.name, periodStart: x.period_start ?? x.periodStart, periodEnd: x.period_end ?? x.periodEnd, analyticAccountId: x.analytic_account_id ?? x.analyticAccountId, plannedAmount: n(x.planned_amount ?? x.plannedAmount) });
...
export function DataProvider({ children }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (user.role === 'User') {
        ...
        const [contacts, products, accounts, journals, analyticAccounts, budgets, purchaseOrders, vendorBills, salesOrders, customerInvoices, payments, journalEntries] = await Promise.all([
          api.get('/contacts?includeArchived=true'), api.get('/products'), api.get('/accounts'), api.get('/journals'), api.get('/analytic-accounts'), api.get('/budgets'), api.get('/purchase-orders'), api.get('/vendor-bills'), api.get('/sales-orders'), api.get('/customer-invoices'), api.get('/payments'), api.get('/journal-entries')
        ]);
        setData({ contacts: contacts.map(mapContact), products: products.map(mapProduct), accounts, journals: journals.map(mapJournal), analyticAccounts: analyticAccounts.map(mapAnalytic), budgets: budgets.map(mapBudget), purchaseOrders: purchaseOrders.map(mapPO), vendorBills: vendorBills.map(mapBill), salesOrders: salesOrders.map(mapSO), customerInvoices: customerInvoices.map(mapInvoice), payments: payments.map(mapPayment), journalEntries });
      }
  ...
  const archiveProduct = useCallback(async (id) => { const updated = await api.patch(`/products/${id}/archive`); await load(); return updated; }, [load]);
  ...
  const addBudget = useCallback(async (b) => { const r = await api.post('/budgets', { ...b, periodStart: b.periodStart, periodEnd: b.periodEnd, analyticAccountId: Number(b.analyticAccountId), plannedAmount: n(b.plannedAmount) }); await load(); return r; }, [load]);
  ...
  const value = useMemo(() => ({ ...data, loading, error, refresh: load, addContact, updateContact, archiveContact, addProduct, updateProduct, archiveProduct, addAccount, updateAccount, addJournal, addAnalyticAccount, addBudget, createPurchaseOrder, convertPOToBill, confirmPurchaseOrder, createSalesOrder, generateInvoiceFromSO, confirmSalesOrder, recordPayment, accountBalance, budgetActual }), [data, loading, error, load, addContact, updateContact, archiveContact, addProduct, updateProduct, archiveProduct, addAccount, updateAccount, addJournal, addAnalyticAccount, addBudget, createPurchaseOrder, convertPOToBill, confirmPurchaseOrder, createSalesOrder, generateInvoiceFromSO, confirmSalesOrder, recordPayment, accountBalance, budgetActual]);
